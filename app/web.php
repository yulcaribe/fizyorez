...
function render_health(): void
{
    ...
    <title>Kurulum Kontrolü · <?= e(config('app.name')) ?></title><link rel="stylesheet" href="<?= e(asset_url('assets/css/app.css') . '?v=8') ?>"></head>
...
{
    ?><!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title><?= e($title) ?> · <?= e(config('app.name')) ?></title><link rel="stylesheet" href="<?= e(asset_url('assets/css/app.css') . '?v=8') ?>"></head><body class="auth-page"><?php
}
...
function render_page(array $user, string $path, ?array $flash): void
{
    ...
    ?>
    <!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
    <title><?= e($title) ?> · <?= e(config('app.name')) ?></title><link rel="stylesheet" href="<?= e(asset_url('assets/css/app.css') . '?v=8') ?>"></head><body>
    ...
    <script src="<?= e(asset_url('assets/js/app.js') . '?v=7') ?>"></script></body></html>
    <?php
}
...
function render_dashboard(array $user): void
{
    if ($user['role'] === 'customer') {
        ...
        $walletAvailable = WalletService::availableBalance((int) $user['id']);
        $isWalletDebt = $walletBalance < 0;
...
function render_users(array $actor, bool $customersOnly): void
{
    ...
    <div class="user-directory <?= $customersOnly ? 'has-wallet' : '' ?>">
        ...
        <?php foreach ($users as $item):
            ...
            $wallet = $wallets[(int) $item['id']] ?? ['wallet_balance' => 0, 'reserved_balance' => 0, 'available_balance' => 0];
            $walletBalance = (float) $wallet['wallet_balance'];
            $walletReserved = (float) $wallet['reserved_balance'];
            $walletDetail = $walletBalance < 0
                ? 'Borç bakiyesi'
                : ($walletReserved > 0 ? money($wallet['available_balance']) . ' kullanılabilir · ' . money($walletReserved) . ' rezerve' : 'Kullanılabilir bakiye');
            if ($editable): ?><details class="user-row"><summary class="user-row-summary"><?php else: ?><div class="user-row is-static"><div class="user-row-summary"><?php endif; ?>
                ...
                <?php if ($customersOnly): ?><span class="user-cell wallet-cell" data-label="Bakiye"><strong class="wallet-amount<?= $walletBalance < 0 ? ' is-negative' : '' ?>"><?= money($walletBalance) ?></strong><small><?= e($walletDetail) ?></small></span><?php endif; ?>
...
function render_customer_wallet(array $user): void
{
    ...
    $spendable = WalletService::spendableBalance($customerId);
    ...
    <section class="wallet-hero financial-wallet<?= $isDebt ? ' has-debt' : '' ?>"><div><p class="eyebrow"><?= $isDebt ? 'Borç bakiyesi' : 'FizyoRez bakiyesi' ?></p><h2><?= money($isDebt ? $balance : $available) ?></h2><p><?= $isDebt ? 'Yönetim tarafından tanımlanan paketlerden kalan ödenmemiş bakiye.' : 'Paket ve tek seans satın alımlarında kullanılabilir bakiye.' ?></p></div><div class="wallet-breakdown"><span><small>Hesap bakiyesi</small><strong><?= money($balance) ?></strong></span><span><small>Onayda rezerve</small><strong><?= money($reserved) ?></strong></span></div></section>
    <?php if ($isDebt): ?><aside class="debt-notice" role="status"><strong><?= money(abs($balance)) ?> ödenmemiş bakiye</strong><span>Yapacağınız bakiye yüklemeleri önce bu borcu kapatır.</span></aside><?php endif; ?>
    <section class="grid two-col"><form method="post" id="wallet-topup" class="panel card-payment form-grid single"><?= csrf_field() ?><input type="hidden" name="action" value="simulate_wallet_topup"><p class="eyebrow">Beta ödeme</p><h2>Kartla bakiye yükle</h2><label>Kart üzerindeki ad<input name="cardholder" autocomplete="cc-name" required></label><label>Kart numarası<input name="card_number" inputmode="numeric" autocomplete="cc-number" maxlength="23" placeholder="0000 0000 0000 0000" data-card-number required></label><div class="form-grid compact"><label>Son kullanma<input name="expiry" inputmode="numeric" autocomplete="cc-exp" maxlength="5" placeholder="AA/YY" data-card-expiry required></label><label>CVV<input type="password" name="cvv" inputmode="numeric" autocomplete="cc-csc" maxlength="4" required></label></div><label>Yüklenecek tutar<input type="number" name="amount" min="1" max="100000" step="0.01" value="1000" required></label><button class="btn btn-primary">Bakiye Yüklemeyi Onaya Gönder</button><p class="form-note">Kart doğrulanırsa işlem yetkili onayına düşer. Tam kart numarası ve CVV kaydedilmez.</p></form>
...
{
    ?><!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="stylesheet" href="<?= e(asset_url('assets/css/app.css') . '?v=8') ?>"><title>FizyoRez Hata</title></head><body class="auth-page"><main class="auth-card wide"><div class="brand-mark">FR</div><h1>İşlem tamamlanamadı</h1><p><?= e(friendly_error_message($e)) ?></p><p class="muted">Kurulum ayrıntıları için <a href="<?= e(url_for('/health')) ?>">sistem kontrolünü</a> açabilirsiniz.</p><a class="btn btn-primary" href="<?= e(url_for('/login')) ?>">Girişe dön</a></main></body></html><?php
}
