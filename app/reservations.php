...
final class ReservationService
{
    ...
    public static function create(array $actor, array $data): array
    {
        ...
        if (!$customer || !$consultant || !$service) {
            throw new RuntimeException('Danışan, danışman veya hizmet bulunamadı.');
        }
        ...
        try {
            ...
            if ($reservationType === 'package') {
                ...
                if (self::creditPolicy() === 'on_booking') {
                    self::deductCredit($customerPackageId);
                    $creditsDeducted = 1;
                } elseif (self::availablePackageCredits($package) < 1) {
    ...
    public static function reschedule(array $actor, int $reservationId, string $newStartsAt): array
    {
        ...
        if ($actor['role'] !== 'admin' && self::isPastChangeDeadline((string) $reservation['starts_at'], (int) $reservation['consultant_id'])) {
    ...
    public static function cancel(array $actor, int $reservationId): array
    {
        ...
        DB::pdo()->beginTransaction();
        try {
            if ((int) $reservation['credits_deducted'] === 1 && (!$late || !$burnLateCredit)) {
                self::returnCredit((int) $reservation['customer_package_id']);
            }
    ...
    {
        if (!in_array($actor['role'], ['admin', 'consultant'], true)) {
        ...
        DB::pdo()->beginTransaction();
        try {
            if (in_array($status, ['completed', 'no_show'], true) && $reservation['reservation_type'] === 'package' && (int) $reservation['credits_deducted'] === 0) {
                self::deductCredit((int) $reservation['customer_package_id']);
    ...
    private static function assertSlotAvailable(int $customerId, int $consultantId, array $service, string $startsAt, string $endsAt, ?int $ignoreReservationId = null): void
    {
        ...
        $params = [$customerId, $startsAt, $endsAt];
        ...
        if ($serviceType === 'group') {
            $conflictParams = [$consultantId, $startsAt, $endsAt, $service['id'], $startsAt, $endsAt];
            ...
            if (DB::fetch($conflictSql, $conflictParams)) {
                throw new RuntimeException('Danışmanın bu saat aralığında başka bir dersi var.');
            }
        ...
        $consultantParams = [$consultantId, $startsAt, $endsAt];
        if ($ignoreReservationId) {
        ...
        if ($consultantConflict) {
            throw new RuntimeException('Danışmanın bu saat aralığında başka rezervasyonu var.');
        }
    }
    ...
    private static function assertConsultantAvailability(int $consultantId, string $startsAt, string $endsAt): void
    {
        ...
        if (!$available) {
            throw new RuntimeException('Danışman bu saat aralığında müsait değil.');
        }
        ...
        if ($timeOff) {
            throw new RuntimeException('Danışmanın bu saat aralığında izin/molası var.');
        }
    }
    ...
    private static function deductCredit(int $customerPackageId): void
    {
        $affected = DB::execute(
            'UPDATE customer_packages SET credits_remaining = credits_remaining - 1
             WHERE id = ? AND credits_remaining > 0',
            [$customerPackageId]
        );

        if ($affected < 1) {
            throw new RuntimeException('Paket hakkı düşülemedi; hak kalmamış olabilir.');
        }
    }

    private static function returnCredit(int $customerPackageId): void
    {
        ...
        DB::execute(
            'UPDATE customer_packages SET credits_remaining = LEAST(credits_total, credits_remaining + 1) WHERE id = ?',
            [$customerPackageId]
        );
    }
